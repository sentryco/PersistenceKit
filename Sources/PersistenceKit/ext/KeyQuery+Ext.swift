...
extension KeyQuery {
   /**
    * Retrieve the private key data from the KeyChain using the specified private key name.
    * - Description: This method retrieves the private key data from the KeyChain using the specified private key name. If the key exists, it returns the key data; otherwise, it returns nil.
    * - Remark: This can be static because the store is in KeyChain.
    * - Throws an error if the key data cannot be retrieved.
    * - Parameter privKeyName: The name of the private key to retrieve.
    * - Returns: The retrieved private key data, or nil if the key does not exist in the KeyChain.
    */
   internal static func getKeyData(privKeyName: String) throws -> Data? {
      // Get the KeyChain query for the specified private key name
      let keyQuery: KeyQuery = .getKeyQuery(key: privKeyName)
      // Try to read the private key data from the KeyChain using the query
      return try Key.read(keyQuery) as? Data // Get priv-key from KeyChain if it exists
   }
   /**
